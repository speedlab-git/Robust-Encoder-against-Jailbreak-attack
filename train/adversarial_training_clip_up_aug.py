...
class Cutout(object):
    ...
    def __call__(self, img):
        ...
        """
        # Convert PIL Image to numpy array to manipulate the image
        img = np.array(img)
        h, w = img.shape[:2]  # Correctly accessing the dimensions of the image
        ...
        # Ensure mask is in the correct shape to apply to all color channels
        mask = torch.from_numpy(mask)
        mask = mask.unsqueeze(2)  # Add an extra dimension for the channels
        mask = mask.expand(-1, -1, 3)  # Expand the mask to cover all 3 channels

        img = img * mask.numpy()  # Apply mask to image
...
def main(args):
    ...
    augmented_transforms = transforms.Compose([
        ...
        transforms.ToTensor()
        # Exclude the original normalization transform
    ])
    ...
    dataloader_2 = DataLoader(dataset_2, batch_size=args.batch_size, shuffle=True, num_workers=8, drop_last=True)
...
def train_one_epoch(
        ...
        embedding_text_labels_norm, args, epoch, dataloader_eval=None, dataloader_2=None
):
    ...
    for i, ((data, targets), (data_2, targets_2)) in enumerate(zip(dataloader, dataloader_2)):
    # Move data to GPU
        data, data_2 = data.cuda(), data_2.cuda()

        is_classification = isinstance(targets, torch.Tensor)
        is_classification_2 = isinstance(targets_2, torch.Tensor)
        data = data.cuda()
        data_2 = data_2.cuda()
        ...
        if is_classification_2:
            targets_2 = targets_2.cuda()        

        with torch.no_grad():
            embedding_orig = model_orig(vision=data_2, output_normalize=args.output_normalize)
