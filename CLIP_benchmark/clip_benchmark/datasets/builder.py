...
def build_wds_dataset(dataset_name, transform, split="test", data_dir="root", cache_dir=None):
    ...
    def read_txt(fname):
        if "://" in fname:
            ...
            stream = os.popen(f"curl -L -s --fail {fname}", "r")
            value = stream.read()
            print("value working?? :", value)
            if stream.close():
    ...
    type_fname = type_fname.replace('\\', '/')
    print("type_fname:", type_fname)
    try:
    ...
    if dataset_type == "retrieval":
        ...
        classnames_fname = classnames_fname.replace('\\', '/')
        print("classnames_fname:", classnames_fname)
        try:
        ...
        templates_fname = templates_fname.replace('\\', '/')
        print("templates_fname:", templates_fname)
        try:
